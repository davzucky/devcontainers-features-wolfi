...
Install the DevContainer CLI, then run `test/run-local.sh -f mise claude cursor opencode pi t3code --skip-autogenerated --skip-duplicated`. The runner tests a temporary copy with this repository's dependencies resolved locally, so unpublished feature changes are included.
