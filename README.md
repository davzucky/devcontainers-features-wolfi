...
- [codex](./src/codex/README.md) - Installs OpenAI Codex CLI through mise on Wolfi base images
...
- [mise](./src/mise/README.md) - Installs mise with project-aware tool versions and shared image defaults
...
## Testing local feature changes

Install the DevContainer CLI, then run `test/run-local.sh -f mise claude codex cursor opencode pi t3code --skip-autogenerated --skip-duplicated`. The runner tests a temporary copy with this repository's dependencies resolved locally, so unpublished feature changes are included.
